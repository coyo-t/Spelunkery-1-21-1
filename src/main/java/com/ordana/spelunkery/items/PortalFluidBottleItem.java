...
public class PortalFluidBottleItem extends HoneyBottleItem
{
	...
	public void appendHoverText (@NotNull ItemStack stack, @Nullable Level level, @NotNull List<Component> tooltip, @NotNull TooltipFlag context)
	{
		if (ClientConfigs.ENABLE_TOOLTIPS.get())
		{
			...
			if (getBoolean(stack))
				tooltip.add(Component.translatable("tooltip.spelunkery.rhymes_with_tears_0").setStyle(Style.EMPTY.applyFormat(ChatFormatting.DARK_PURPLE)));
			else
				tooltip.add(Component.translatable("tooltip.spelunkery.rhymes_with_tears_1", getBoolean(stack)).setStyle(Style.EMPTY.applyFormat(ChatFormatting.DARK_PURPLE)));
			...
			if (InputConstants.isKeyDown(Minecraft.getInstance().getWindow().getWindow(), Minecraft.getInstance().options.keyShift.key.getValue()))
			{
				...
				if (CommonConfigs.CRYING_OBSIDIAN_PORTAL_FLUID.get() && CommonConfigs.RESPAWN_ANCHOR_PORTAL_FLUID.get())
					tooltip.add(Component.translatable("tooltip.spelunkery.portal_fluid_3c").setStyle(Style.EMPTY.applyFormat(ChatFormatting.GRAY)));
				else if (CommonConfigs.CRYING_OBSIDIAN_PORTAL_FLUID.get())
					tooltip.add(Component.translatable("tooltip.spelunkery.portal_fluid_3a").setStyle(Style.EMPTY.applyFormat(ChatFormatting.GRAY)));
				else if (CommonConfigs.RESPAWN_ANCHOR_PORTAL_FLUID.get())
					tooltip.add(Component.translatable("tooltip.spelunkery.portal_fluid_3b").setStyle(Style.EMPTY.applyFormat(ChatFormatting.GRAY)));
			}
			else
			{
