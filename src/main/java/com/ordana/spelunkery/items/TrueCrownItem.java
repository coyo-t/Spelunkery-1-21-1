	...
	public void inventoryTick (@NotNull ItemStack stack, @NotNull Level level, Entity entity, int slotId, boolean isSelected)
	{
		if (entity instanceof Player lEntity)
		{
			if (lEntity.getItemBySlot(EquipmentSlot.HEAD).getItem() == this)
			{
				...
				if (level.random.nextFloat() < 0.1 && (lEntity.getDeltaMovement().x > 0 || lEntity.getDeltaMovement().z > 0))
					level.playLocalSound(entity.blockPosition(), ModSoundEvents.JANGLE.get(), SoundSource.PLAYERS, 1.0f, 1.0f + (level.random.nextFloat() / 2), false);
			}
		}
