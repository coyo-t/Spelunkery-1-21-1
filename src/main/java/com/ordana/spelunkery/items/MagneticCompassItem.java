...
public class MagneticCompassItem extends Item
{
	...
	public InteractionResultHolder<ItemStack> use (@NotNull Level level, @NotNull Player player, InteractionHand hand)
	{
		...
		if (compoundTag.contains("magnetitePos"))
		{
			...
			player.displayClientMessage(Component.translatable("tooltip.spelunkery.player_pos", getPlayerX(stack), getPlayerZ(stack)).setStyle(Style.EMPTY.applyFormat(ChatFormatting.DARK_GREEN)), true);
