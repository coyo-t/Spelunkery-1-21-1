...
public class GrowableMushroomBlock extends ModMushroomBlock implements BonemealableBlock
{
	...
	public boolean growMushroom (ServerLevel level, BlockPos pos, BlockState state, RandomSource random)
	{
		...
		if (state.is(ModBlocks.INKCAP_MUSHROOM.get()))
			feature = (level.registryAccess().registry(Registries.CONFIGURED_FEATURE).get().getHolder(
					  ResourceKey.create(Registries.CONFIGURED_FEATURE, Spelunkery.res("huge_inkcap_mushroom_bonemeal"))).get());
		if (state.is(ModBlocks.WHITE_INKCAP_MUSHROOM.get()))
			feature = (level.registryAccess().registry(Registries.CONFIGURED_FEATURE).get().getHolder(
					  ResourceKey.create(Registries.CONFIGURED_FEATURE, Spelunkery.res("huge_white_inkcap_mushroom_bonemeal"))).get());
		if (state.is(ModBlocks.PORTABELLA.get()))
			feature = (level.registryAccess().registry(Registries.CONFIGURED_FEATURE).get().getHolder(
