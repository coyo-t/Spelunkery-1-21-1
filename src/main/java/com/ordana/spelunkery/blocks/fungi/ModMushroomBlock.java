		...
		if (random.nextInt(25) == 0)
		{
			...
			if (level.isEmptyBlock(blockPos2) && state.canSurvive(level, blockPos2) && !state.is(ModBlocks.CRIMINI.get()) && !state.is(ModBlocks.BUTTON_MUSHROOM.get()))
			{
				if (state.is(ModBlocks.PORTABELLA.get()))
					level.setBlock(blockPos2, ModBlocks.BUTTON_MUSHROOM.get().defaultBlockState(), 2);
				else if (!state.is(ModBlocks.CRIMINI.get()) && !state.is(ModBlocks.BUTTON_MUSHROOM.get()))
