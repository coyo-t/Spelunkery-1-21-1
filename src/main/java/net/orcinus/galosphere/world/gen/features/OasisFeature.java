        ...
        for (int x = -xRadius; x <= xRadius; x++) {
            for (int z = -zRadius; z <= zRadius; z++) {
                for (int y = -yRadius; y <= 0; y++) {
                    ...
                    Structure structure = world.registryAccess().registryOrThrow(Registries.STRUCTURE).get(Galosphere.id("pink_salt_shrine"));
                    if (structure != null && world.getLevel().structureManager().getStructureWithPieceAt(pos, structure).isValid()) {
                        return false;
                    }
                    double xSquared = x * x;
                    double ySquared = y * y;
